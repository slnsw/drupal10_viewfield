...
namespace Drupal\Tests\field\Kernel\Viewfield;

use Drupal\Core\Cache\Cache;
use Drupal\Core\Cache\CacheableMetadata;
use Drupal\Core\Field\FieldStorageDefinitionInterface;
use Drupal\Core\Field\Plugin\Field\FieldFormatter\ViewfieldEntityFormatter;
...
use Drupal\Tests\field\Traits\ViewfieldTestTrait;
 ...
 * @group viewfield
 */
class ViewfieldFormatterTest extends EntityKernelTestBase {
  ...
  protected function setUp() {
    ...
    $this->createViewfieldField($this->entityType, $this->bundle, $this->fieldName, 'Field test', $this->entityType, 'default', [], FieldStorageDefinitionInterface::CARDINALITY_UNLIMITED);
  ...
  public function testAccess() {
    ...
    foreach ($formatter_manager->getOptions('viewfield') as $formatter => $name) {
  ...
  public function testCustomCacheTagFormatter() {
    ...
    $formatter = 'viewfield_custom_cache_tag';
  ...
  public function testEntityFormatter() {
    /** @var \Drupal\Core\Render\RendererInterface $renderer */
    $renderer = $this->container->get('renderer');
    $formatter = 'viewfield_entity_view';
    $build = $this->buildRenderArray([$this->referencedEntity, $this->unsavedReferencedEntity], $formatter);
...
';
  ...
  public function testEntityFormatterRecursiveRendering() {
    ...
    // Set the default view mode to use the 'viewfield_entity_view'
    // formatter.
    entity_get_display($this->entityType, $this->bundle, 'default')
    ...
    // reference field.
    $expected_occurrences = ViewfieldEntityFormatter::RECURSIVE_RENDER_LIMIT * 2 + 2;
   ...
   */
  public function testViewfieldRecursiveProtectionWithManyRenderedEntities() {
    $formatter = 'viewfield_entity_view';
    $view_builder = $this->entityManager->getViewBuilder($this->entityType);
